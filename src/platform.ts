...
export class RoombaMatterbridgePlatform extends MatterbridgeDynamicPlatform {
  ...
  override async onConfigChanged(config: PlatformConfig): Promise<void> {
    ...
    const resolvedBlidByIp = new Map(
      (this.platformConfig.devices ?? [])
        .filter((d) => d.blid && d.ipAddress)
        .map((d) => [d.ipAddress, d.blid] as [string, string]),
    );
  ...
  private async runApplyDiscoveredRooms(): Promise<void> {
    const devices = this.platformConfig.devices ?? [];
    this.log.info(
      `applyDiscoveredRooms triggered. ${devices.length} device(s) in config, ` +
    ...
    for (const deviceConfig of devices) {
      ...
      if (!connection) {
        this.log.warn(
          `[${deviceConfig.name ?? deviceConfig.blid ?? '(no blid)'}] No active connection found ` +
