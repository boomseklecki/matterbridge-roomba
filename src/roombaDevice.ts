...
export class RoombaDevice {
  ...
  constructor(info: DeviceInfo, globalConfig: PlatformConfig, log: AnsiLogger) {
    ...
    vacuumEndpoint.createDefaultBridgedDeviceBasicInformationClusterServer(
      info.name,
      info.blid,
      0x1234,
      'iRobot',
  ...
  private updateEndpointAttributes(status: Status): void {
    ...
    this.endpoint.updateAttribute('rvcRunMode', 'currentMode', runMode, this.log).catch((e: Error) => {
      this.log.debug('updateAttribute rvcRunMode failed: %s', e.message)
    })

    const opState = this.toOperationalState(status)
    this.endpoint.updateAttribute('rvcOperationalState', 'operationalState', opState, this.log).catch((e: Error) => {
    ...
    if (status.batteryLevel !== undefined) {
      ...
      this.endpoint.updateAttribute('powerSource', 'batPercentRemaining', batPct, this.log).catch((e: Error) => {
