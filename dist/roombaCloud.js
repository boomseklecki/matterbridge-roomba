...
async function fetchJson(url, init) {
    ...
    try {
        ...
        throw new RoombaCloudError('network', `Network error calling ${url}: ${err.message}`, { cause: err });
    }
    finally {
        clearTimeout(timer);
    }
...
// --- Step 1: discovery -------------------------------------------------------
async function discoverEndpoints(countryCode) {
...
// --- Step 2: Gigya login -----------------------------------------------------
async function gigyaLogin(email, password, apiKey, datacenter) {
    ...
    if (innerError === 403042) {
        ...
        throw new RoombaCloudError('gigya_captcha_required', 'Gigya is requiring a captcha. Sign in once via the official iRobot app to clear it, then retry.', { statusCode: innerStatus, details: body });
    }
    if (innerError !== 0) {
        throw new RoombaCloudError('gigya_other', `Gigya error ${innerError}: ${body.errorMessage ?? 'unknown'} (${body.errorDetails ?? ''})`, { statusCode: innerStatus, details: body });
    }
...
async function iRobotLogin(httpBase, gigya) {
 ...
 */
export async function getRoombaCloudCredentials(input) {
...
//# sourceMappingURL=roombaCloud.js.map