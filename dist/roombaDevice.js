...
export class RoombaDevice {
    ...
    constructor(info, globalConfig, log) {
        ...
        this.debugMode = globalConfig.debug ?? false;
    ...
    static EVERYWHERE_AREA_ID = 1;
    selectedMissions() {
        const selected = this.endpoint.getAttribute('serviceArea', 'selectedAreas');
        if (!selected || selected.length === 0 || selected.includes(RoombaDevice.EVERYWHERE_AREA_ID))
            return [];
    ...
    async startClean(roomba) {
        ...
        if (withFavorite.length > 0) {
            if (withFavorite.length > 1 || withoutFavorite.length > 0) {
                this.log.warn('Multiple missions selected with favorite_id — running first (%s), ignoring others', withFavorite[0].name);
            }
    ...
    receiveRobotState(state) {
        if (this.receivedRobotStateIsComplete(state)) {
            ...
            const missing = ['batPct', 'bin', 'cleanMissionStatus'].filter(f => state[f] === undefined);
                ...
                const updateState = (state) => {
                    if (this.receivedRobotStateIsComplete(state)) {
                        ...
                        if (this.debugMode && !this.lastCommandLogged) {
                            ...
                            roomba.getRobotState(['lastCommand']).then((s) => {
                                if (s.lastCommand) {
                            ...
                            }).catch((e) => this.log.debug('lastCommand query failed: %s', e.message));
                        }
                        resolve();
                        callback();
                    }
