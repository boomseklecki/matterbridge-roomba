    ...
    async runApplyDiscoveredRooms() {
        ...
        for (const deviceConfig of devices) {
            ...
            if (discovered.length === 0) {
                this.log.info(`[${deviceConfig.name ?? deviceConfig.blid}] No rooms discovered yet. ` +
        ...
        for (const deviceConfig of devices) {
            ...
            if (discoveredFavorites.length === 0)
                continue;
            const existingFavoriteIds = new Set((deviceConfig.rooms ?? []).map((r) => r.favoriteId).filter(Boolean));
            let addedCount = 0;
            for (const fav of discoveredFavorites) {
                if (existingFavoriteIds.has(fav.favoriteId))
                    continue;
            ...
            if (addedCount > 0) {
                const deviceLabel = deviceConfig.name ?? deviceConfig.blid;
                this.log.info(`[${deviceLabel}] Added ${addedCount} saved mission(s). Rename them in the config UI, then restart the plugin.`);
            }
        }
        if (updatedAny) {
            try {
